...
const generateProducts = (count) => {
  const products = [];
  const productNames = ["Classic Loafer", "Stylish Sneaker", "Leather Oxford", "Running Shoe", "Hiking Boot", "Sandals", "Winter Boots", "Formal Pumps", "Athletic Trainers", "Slip-on Moccasin"];
...
const Sidebar = ({ activeTab, setActiveTab }) => (
  <aside className="w-64 bg-black text-white flex flex-col h-full rounded-l-2xl shadow-xl">
    ...
    <nav className="flex-1 mt-8 space-y-2 px-4">
      ...
      <NavItem icon={ShoppingBag} label="Products" isActive={activeTab === 'products'} onClick={() => setActiveTab('products')} />
      <NavItem icon={BarChart} label="Reports" isActive={activeTab === 'reports'} onClick={() => setActiveTab('reports')} />
...
const NavItem = ({ icon: Icon, label, isActive, onClick }) => (
  <button
    ...
    className={`w-full flex items-center p-3 rounded-xl transition-colors duration-200 
      ${isActive ? 'bg-white font-semibold text-black' : 'hover:bg-white hover:text-gray-600'}`}
  >
    ...
    <div className="flex items-center space-x-4">
      <div className="flex items-center space-x-2 text-sm text-gray-600">
        <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
          <img src="https://api.dicebear.com/7.x/pixel-art/svg?seed=admin" alt="Admin" />
        </div>
        <div>
...
const AdminPanelApp = () => {
    ...
    const fetchProducts = async () => {
        ...
        setError(null);
        try {
        // Assuming your backend is running at http://localhost:5000/api/products
    ...
    const handleAddProduct = async (newProduct) => {
        ...
        const response = await axios.post(
            'https://api-shoe-ecommerce.onrender.com/api/v1/auth/products', 
            newProduct
        );
        ...
        setError("Failed to add product. " + (e.response?.data?.message || e.message));
        }
    };
